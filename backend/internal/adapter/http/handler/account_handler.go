...
// UpdateName handles PUT /accounts/{id}.
func (h *AccountHandler) UpdateName(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.logger.WarnContext(
			r.Context(),
			"invalid account id format in update",
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		h.logger.WarnContext(
			r.Context(),
			"failed to decode account update request body",
			slog.String("error", err.Error()),
		)
		writeJSON(w, http.StatusBadRequest, ErrorResponse{
			Error:   "INVALID_REQUEST",
	...
	if err := h.validate.Struct(req); err != nil {
		h.logger.WarnContext(
			r.Context(),
			"account update validation failed",
			slog.String("error", err.Error()),
		)
		writeJSON(w, http.StatusBadRequest, ErrorResponse{
			Error:   "VALIDATION_ERROR",
	...
	if err := h.service.UpdateName(r.Context(), id, req.Name); err != nil {
		switch {
		...
		case errors.Is(err, account.ErrAccountNameExists):
			h.logger.WarnContext(
				r.Context(),
				"duplicate account name during update",
				slog.String("id", id.String()),
			)
			writeJSON(w, http.StatusConflict, ErrorResponse{
				Error:   "CONFLICT",
	...
	writeJSON(w, http.StatusOK, SuccessResponse{Message: "Account updated successfully"})
}

// Archive handles POST /accounts/{id}/archive.
func (h *AccountHandler) Archive(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.logger.WarnContext(
			r.Context(),
			"invalid account id format in archive",
			slog.String("raw_id", rawID),
		)
		writeJSON(w, http.StatusBadRequest, ErrorResponse{
			Error:   "INVALID_REQUEST",
	...
	if err := h.service.Archive(r.Context(), id); err != nil {
		if errors.Is(err, account.ErrAccountNotFound) {
			h.logger.WarnContext(
				r.Context(),
				"account not found during archive",
				slog.String("id", id.String()),
			)
			writeJSON(w, http.StatusNotFound, ErrorResponse{
				Error:   "NOT_FOUND",
	...
	writeJSON(w, http.StatusOK, SuccessResponse{Message: "Account archived successfully"})
}

// Unarchive handles POST /accounts/{id}/unarchive.
func (h *AccountHandler) Unarchive(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		h.logger.WarnContext(
			r.Context(),
			"invalid account id format in unarchive",
			slog.String("raw_id", rawID),
		)
		writeJSON(w, http.StatusBadRequest, ErrorResponse{
			Error:   "INVALID_REQUEST",
	...
	if err := h.service.Unarchive(r.Context(), id); err != nil {
		if errors.Is(err, account.ErrAccountNotFound) {
			h.logger.WarnContext(
				r.Context(),
				"account not found during unarchive",
				slog.String("id", id.String()),
			)
			writeJSON(w, http.StatusNotFound, ErrorResponse{
				Error:   "NOT_FOUND",
		...
		writeJSON(w, http.StatusInternalServerError, ErrorResponse{
			Error:   "INTERNAL_ERROR",
			Message: "Failed to unarchive account",
			Code:    "ERR_INTERNAL_SERVER",
		})
		return
	}

	writeJSON(w, http.StatusOK, SuccessResponse{Message: "Account unarchived successfully"})
}
